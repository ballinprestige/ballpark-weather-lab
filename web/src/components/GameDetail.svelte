...
<article class="game-detail" aria-labelledby={titleId} data-testid="game-detail">
  <header class="game-detail-header">
    ...
    {#if headingLevel === 2}
      <h2 id={titleId}>{teamLabel(game.away_team)} <span>at</span> {teamLabel(game.home_team)}</h2>
    {:else}
      <h3 id={titleId}>{teamLabel(game.away_team)} <span>at</span> {teamLabel(game.home_team)}</h3>
    {/if}
  ...
  <section class="conditions-strip" aria-labelledby={`conditions-${game.game_pk}`}>
    ...
    <dl class="reading-grid">
      <div><dt>Temperature</dt><dd>{game.weather.temperature_f.toFixed(0)}°F</dd></div>
      <div><dt>Humidity</dt><dd>{game.weather.humidity_pct.toFixed(0)}%</dd></div>
      <div><dt>Wind</dt><dd>{windLabel(game.weather)}</dd></div>
