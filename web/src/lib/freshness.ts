...
export function assessOddsFreshness(
  odds: { state: 'current' | 'stale' | 'observed_unknown_age' | 'unavailable'; reason: string | null; source_updated_at: string | null; observed_at: string | null; line?: number | null; over_price?: number | null; under_price?: number | null },
  now = new Date()
): OddsFreshness {
  if (odds.state === 'unavailable') {
    return { state: 'unavailable', reason: odds.reason };
  }
  if (odds.state === 'observed_unknown_age') {
    const observedMs = odds.observed_at ? Date.parse(odds.observed_at) : NaN;
    if (!Number.isFinite(observedMs) || observedMs > now.getTime() + QUOTE_FUTURE_SKEW_MS) {
      return { state: 'unavailable', reason: 'Observed market retrieval time is missing or ahead of the comparison clock.' };
    }
    return { state: 'observed', reason: odds.reason ?? 'Source quote-update age is unverified.' };
  }
