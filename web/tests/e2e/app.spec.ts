...
test('desktop employer path exposes the complete evidence chain', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('h1')).toHaveText('The Slate');
  await expect(page.getByRole('heading', { name: 'Wind Field Strip' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Ledger mode' })).toBeVisible();
  ...
  await expect(page.locator('meta[name="theme-color"]')).toHaveAttribute('content', '#0e1215');
  ...
  await page.getByRole('link', { name: /Open San Diego Padres.*San Francisco Giants.*station/i }).click();
...
test('mobile slate and game station remain compact and touch safe', async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.startsWith('mobile'));
  await mockPublication(page, readyPayload());
  await page.goto('/#slate');

  const second = page.getByRole('link', { name: /Open San Diego Padres.*San Francisco Giants.*station/i });
  await expect(page.locator('h1')).toHaveText('The Slate');
  await expect(page.getByTestId('game-detail')).toHaveCount(0);
  const [gridBox, windBox] = await Promise.all([
    page.locator('.heritage-game-grid').boundingBox(),
  ...
  await expect(page.getByRole('link', { name: 'Back to The Slate' })).toBeVisible();
  expect(await page.evaluate(() => window.scrollY)).toBe(0);
  await expect(stationHeading).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Park wind diagram' })).toBeVisible();

  const tooSmall = await page.locator('button:visible, nav a:visible, .heritage-game-card h3 a:visible, .station-back:visible').evaluateAll((elements) => elements
  ...
  await page.getByRole('link', { name: 'Back to The Slate' }).click();
...
test('single-game mobile wind strip keeps its station in view', async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.startsWith('mobile'));
  const payload = readyPayload();
...
test('a full fifteen-game slate keeps the original card grid and ledger paths', async ({ page }, testInfo) => {
  test.skip(!testInfo.project.name.startsWith('desktop'));
  await mockPublication(page, fifteenGamePayload());
  await page.goto('/#slate');

  const cards = page.locator('.heritage-game-card');
  ...
  await page.getByRole('button', { name: 'Ledger mode' }).click();
...
test('a missing-weather game is held without hiding its valid neighbor', async ({ page }, testInfo) => {
  ...
  for (const option of ['time', 'wind', 'venue', 'movement']) {
    ...
    await expect(page.locator('.heritage-game-card').last().getByRole('link')).toHaveAccessibleName(/San Diego Padres.*San Francisco Giants/i);
  }
  await page.getByRole('button', { name: 'Open air' }).click();
  await expect(page.getByRole('link', { name: /Open San Diego Padres.*San Francisco Giants.*station/i })).toHaveCount(0);
  await page.getByRole('button', { name: 'Incomplete' }).click();
  await expect(page.locator('.heritage-game-card')).toHaveCount(1);
  await expect(page.locator('.heritage-game-card').getByRole('link')).toHaveAccessibleName(/San Diego Padres.*San Francisco Giants/i);
...
test('a release hash mismatch fails closed before slate values render', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('The Slate', { exact: true })).toHaveCount(0);
