...
_TIME_RE = re.compile(
    r'<div\b[^>]*class="[^"]*game-time[^"]*"[^>]*>.*?<span>\s*(\d{1,2}:\d{2})\s*</span>',
    re.I | re.S,
)
_QUOTE_RE = re.compile(
    r"\b([ou])\s*(\d+(?:\.\d+)?)\s*.*?American\s+__american[^>]*>\s*([+\-]|&#x2B;|&minus;)?\s*(\d{2,4})",
    re.I | re.S,
)
...
class OddsSnapshotCache:
            ...
            try:
                for value in json.loads(path.read_text(encoding="utf-8")).get("markets", []):
                    if isinstance(value, dict):
                        self._by_game[(str(value.get("slate_date")), int(value.get("game_pk")))] = (
                            value
                        )
    ...
    def accept(self, market: dict[str, Any]) -> bool:
        ...
        if existing and str(existing.get("observed_at") or "") >= str(
            market.get("observed_at") or ""
        ):
            return False
...
def unavailable_market(
    game_pk: int, slate_date: date, reason: str, *, observed_at: datetime | None = None
) -> dict[str, Any]:
    return {
        "game_pk": game_pk,
...
def _parse_rows(document: str, book_id: str) -> list[dict[str, Any]]:
    ...
    for row_match in _ROW_RE.finditer(_extract_total_table(document)):
        ...
        for cell_match in _CELL_RE.finditer(row):
            ...
            if (
                "liveOddsCell" not in classes
                or attrs.get("data-book", "").lower() != book_id.lower()
            ):
                continue
            try:
    ...
    *,
    ...
    result = {
        int(game["game_pk"]): unavailable_market(
            int(game["game_pk"]),
            target_date,
            "no matching full-game total from selected book",
    ...
    for game_pk, records in candidates.items():
        ...
        newest_conflicts = {
            ...
            for r in records
            if r["source_updated_at"] == newest["source_updated_at"]
        }
        if len(newest_conflicts) > 1:
            result[game_pk] = unavailable_market(
                game_pk,
                target_date,
                "conflicting selected-book quotes share a source timestamp",
                observed_at=observed_at,
            )
            continue
        if not trust_source_timestamp:
            ...
            result[game_pk] = {
                **unavailable_market(
                    game_pk,
                    target_date,
                    "Covers book-cell timestamp is not documented as this quote's update time",
                    observed_at=observed_at,
                ),
        ...
        if age_seconds < -FUTURE_SKEW_SECONDS:
            result[game_pk] = unavailable_market(
                game_pk,
                target_date,
                "selected-book quote has an implausible future source timestamp",
                observed_at=observed_at,
            )
            continue
        ...
        reason = (
            ...
            if state == "current"
                ...
                f"selected-book quote is {max(0, round(age_seconds))} seconds old; "
                f"freshness limit is {freshness_seconds} seconds"
            )
        )
        public = {
            "game_pk": game_pk,
            ...
            "eligibility": _eligibility(
                next(game for game in schedule if int(game["game_pk"]) == game_pk)
            ),
            "sportsbook_id": book_id.lower(),
            "sportsbook_name": book_id,
            "provider": "Covers",
...
class CoversOddsProvider:
    ...
    def fetch(
        self, target_date: date, schedule: list[dict[str, Any]], *, observed_at: datetime
    ) -> dict[int, dict[str, Any]]:
        try:
            document = self.client.get_bytes(
                f"{COVERS_ODDS_URL}?{urlencode({'date': target_date.isoformat()})}"
            ).decode("utf-8")
            return normalize_covers_html(
                document,
            ...
            return {
                int(game["game_pk"]): unavailable_market(
                    int(game["game_pk"]),
                    target_date,
                    f"Covers full-game-total source unavailable: {type(exc).__name__}: {exc}",
                    observed_at=observed_at,
                )
                for game in schedule
            }
...
class TheOddsApiProvider:
    ...
    def __init__(
        ...
        *,
        api_key: str | None,
        book_id: str,
        requester: Callable[[str], tuple[int, dict[str, str], bytes]] | None = None,
        cache: OddsSnapshotCache | None = None,
        quota_floor: int = 1,
    ) -> None:
        self.client, self.api_key, self.book_id = client, api_key, book_id
        self.requester, self.cache, self.quota_floor = (
            requester,
            cache or OddsSnapshotCache(),
            quota_floor,
        )
    ...
    def fetch(
        ...
        if not self.api_key:
            return {
                int(g["game_pk"]): unavailable_market(
                    int(g["game_pk"]),
                    target_date,
                    "The Odds API runtime key is not configured",
                    observed_at=observed_at,
                )
                for g in schedule
            }
        ...
        for _attempt in range(2):
            try:
                status, headers, body = (
                    self.requester(f"{ODDS_API_URL}?{query}")
                    if self.requester
                    else (200, {}, self.client.get_bytes(f"{ODDS_API_URL}?{query}"))
                )
            except Exception as exc:
                ...
                if (
                    ...
                    and int(remaining) < self.quota_floor
                ):
                    last_reason, retryable = (
                        "The Odds API quota floor reached; no request retry",
                        False,
                    )
                elif status == 200:
                    try:
            ...
            if not retryable:
                break
        return {
            int(g["game_pk"]): self.cache.for_slate(target_date, int(g["game_pk"]))
            or unavailable_market(
                int(g["game_pk"]), target_date, last_reason, observed_at=observed_at
            )
            for g in schedule
        }
        ...
        *,
        ...
        result = {
            int(g["game_pk"]): unavailable_market(
                int(g["game_pk"]),
                target_date,
                "no matching selected-book full-game total",
                observed_at=observed_at,
            )
            for g in schedule
        }
        ...
        for event in events:
            ...
            matches = [
                ...
                and g.get("away_team") == away
                and g.get("game_time")
                and _utc(str(g["game_time"])) == _utc(str(event.get("commence_time")))
            ]
            if len(matches) != 1:
                continue
            ...
            books = [
                ...
                if isinstance(b, dict) and b.get("key") == self.book_id
            ]
            if len(books) != 1:
                continue
            totals = [
                ...
                if isinstance(m, dict) and m.get("key") == "totals"
            ]
            if len(totals) != 1:
                continue
            outcomes = {
                o.get("name"): o for o in totals[0].get("outcomes", []) if isinstance(o, dict)
            }
            over, under = outcomes.get("Over"), outcomes.get("Under")
            if (
                not over
                or not under
                or over.get("point") != under.get("point")
                or not isinstance(over.get("price"), int)
                or not isinstance(under.get("price"), int)
            ):
                continue
            raw = hashlib.sha256(
            ...
            result[game_pk] = {
                **unavailable_market(
                    game_pk,
                    target_date,
                    "bookmaker last_update granularity is not certified as this total quote update",
                    observed_at=_utc(observed_at),
                ),
                "provider_event_id": str(event.get("id") or "") or None,
                "provider": "The Odds API",
                "source_url": "https://the-odds-api.com/liveapi/guides/v4/",
                "sportsbook_id": self.book_id,
                "sportsbook_name": str(books[0].get("title") or self.book_id),
                "line": over["point"],
                "over_price": over["price"],
                "under_price": under["price"],
                "raw_sha256": raw,
                "snapshot_id": hashlib.sha256(f"{raw}:{game_pk}".encode()).hexdigest(),
                "source_schema_version": "the-odds-api-v4-totals",
            }
        return result
