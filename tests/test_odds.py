...
from ballpark.odds import (
    OddsSnapshotCache,
    TheOddsApiProvider,
    normalize_covers_html,
...
def _row(
    *,
    ...
    under: str = "u 8.5",
    under_price: str = "-115",
    board_time: str = "13:05",
) -> str:
    return (
        f'<tr class="oddsGameRow"><td class="left-cell"><div class="game-time">'
        f'<span>Today,</span><span>{board_time}</span></div><div class="td-cell away-cell">'
        f'<strong>NYY</strong></div><div class="td-cell home-cell"><strong>BOS</strong>'
        f'</div></td><td class="liveOddsCell" data-book="bet365" data-game="{event}" '
        f'data-date="{at}"><div class="td-cell away-cell">{over}<span '
        f'class="American __american">{over_price}</span></div><div class="td-cell home-cell">'
        f'{under}<span class="American __american">{under_price}</span></div></td></tr>'
    )
...
def _schedule() -> list[dict[str, object]]:
    return [
        {"game_pk": 11, "home_team": "BOS", "away_team": "NYY", "game_time": "2026-09-06T17:05:00Z"}
    ]
...
def test_normalizes_actual_two_sided_full_game_quote() -> None:
    ...
    markets = normalize_covers_html(
        _document(_row(at=1788714000)),
        target_date=TARGET,
        schedule=_schedule(),
        observed_at=OBSERVED,
        trust_source_timestamp=True,
    )
    market = markets[11]
...
def test_rejects_missing_side_mismatched_line_and_ambiguous_doubleheader() -> None:
    missing_side = normalize_covers_html(
        _document(_row(under="")), target_date=TARGET, schedule=_schedule(), observed_at=OBSERVED
    )[11]
    mismatched_line = normalize_covers_html(
        _document(_row(under="u 9.0")),
        target_date=TARGET,
        schedule=_schedule(),
        observed_at=OBSERVED,
    )[11]
    doubleheader = _schedule() + [
        {"game_pk": 12, "home_team": "BOS", "away_team": "NYY", "game_time": "2026-09-06T17:05:00Z"}
    ]
    ambiguous = normalize_covers_html(
        _document(_row()), target_date=TARGET, schedule=doubleheader, observed_at=OBSERVED
    )
...
def test_stale_out_of_order_and_conflicting_duplicate_handling() -> None:
    stale = normalize_covers_html(
        _document(_row(at=1788710400)),
        target_date=TARGET,
        schedule=_schedule(),
        observed_at=OBSERVED,
        trust_source_timestamp=True,
    ...
    selected = normalize_covers_html(
        _document(newer, older),
        target_date=TARGET,
        schedule=_schedule(),
        observed_at=OBSERVED,
        trust_source_timestamp=True,
    )[11]
    conflict = normalize_covers_html(
        _document(
            newer, _row(event="conflict", at=1788714000, over_price="-110", under_price="-110")
        ),
        target_date=TARGET,
        schedule=_schedule(),
        observed_at=OBSERVED,
...
def test_wrong_book_or_provider_schema_change_is_unavailable() -> None:
    ...
    malformed = (
        '<table id="total-table"><tr class="oddsGameRow">'
        '<td>changed provider shape</td></tr></table>'
    )
    assert (
        normalize_covers_html(
            wrong_book, target_date=TARGET, schedule=_schedule(), observed_at=OBSERVED
        )[11]["state"]
        == "unavailable"
    )
    assert (
        normalize_covers_html(
            malformed, target_date=TARGET, schedule=_schedule(), observed_at=OBSERVED
        )[11]["state"]
        == "unavailable"
    )
...
def test_odds_api_replay_is_exact_book_totals_only_and_honest_about_book_time() -> None:
    ...
    events = [
        {
            "id": "event-1",
            "commence_time": "2026-09-06T17:05:00Z",
            "home_team": "BOS",
            "away_team": "NYY",
            "bookmakers": [
                {
                    "key": "other",
                    "title": "Other",
                    "last_update": "2026-09-06T17:00:00Z",
                    "markets": [],
                },
                {
                    "key": "draftkings",
    ...
    market = provider.normalize(
        events, target_date=TARGET, schedule=_schedule(), observed_at=OBSERVED
    )[11]
    assert market["state"] == "unavailable"
...
def test_odds_api_missing_key_and_malformed_replay_fail_closed() -> None:
    ...
    assert (
        configured.normalize(
            {"unexpected": True}, target_date=TARGET, schedule=_schedule(), observed_at=OBSERVED
        )[11]["state"]
        == "unavailable"
    )
...
def test_provider_failure_classification_and_snapshot_order_are_bounded() -> None:
    ...
    assert (
        cache.accept(first) is True
        and cache.accept(older) is False
        and cache.accept(tomorrow) is True
    )
...
def test_odds_api_execution_retries_once_then_uses_same_date_cache_on_401() -> None:
    ...
    responses = [
        (429, {}, b"[]"),
        (200, {"x-requests-remaining": "5"}, __import__("json").dumps(events).encode()),
    ]
    provider = TheOddsApiProvider(
        object(),
        api_key="runtime-only",
        book_id="draftkings",
        requester=lambda _url: responses.pop(0),
    )
...
def test_kalshi_exchange_asks_are_cents_not_american_odds() -> None:
    ...
    quote = normalize_markets(
        game,
        {"status": "open", "event_ticker": "KXMLBTOTAL-26SEP072110CINLAD"},
        [
            {
                "event_ticker": "KXMLBTOTAL-26SEP072110CINLAD",
                "ticker": "KXMLBTOTAL-26SEP072110CINLAD-9",
                "status": "active",
