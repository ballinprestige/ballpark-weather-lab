...
def worker(tmp_path: Path, calls: list[object], handler: object) -> RuntimeWorker:
    return RuntimeWorker(
        [JobSpec("markets", interval_seconds=60, timeout_seconds=20, max_attempts=2, retry_backoff_seconds=5)],
...
def test_retry_is_bounded_and_failure_never_rewrites_last_good(tmp_path: Path) -> None:
    ...
    assert failing.run_once(now=first_failure + timedelta(seconds=5))["jobs"][0]["state"] == "failed_exhausted"
...
def test_probe_is_read_only_and_fails_closed_for_silent_or_late_worker(tmp_path: Path) -> None:
    ...
    ready = probe_runtime(tmp_path / "state", max_heartbeat_age_seconds=30, max_job_lag_seconds=5, now=NOW)
    stale = probe_runtime(
        tmp_path / "state",
